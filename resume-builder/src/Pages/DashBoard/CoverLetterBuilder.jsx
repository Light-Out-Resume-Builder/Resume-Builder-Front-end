...
const CoverLetterBuilder = () => {
  const steps = ["Select a Template", "Job Goals", "Work info", "Finalize"];
  // const ButtonElement = [];
    ...
    <DashboardLayout>
      <div className="px-[5%] lg:px-[10%] py-10 lg:py-20">
        ...
        <div className="flex justify-around px-2 lg:px-10 mt-12">
          {steps?.map((step, i) => (
            <div
              className="flex items-center flex-col relative justify-center w-[100%] [&:not(:first-child)]:before:content-[''] [&:not(:first-child)]:before:bg-[#1E40AF] before:absolute before:w-full before:h-[10px] before:right-2/4 before:top-[30%] before:-translate-y-2/4"
              key={i}
            >
              <div
                className={`w-7 h-7 flex items-center  justify-center z-10 relative rounded-full bg- ${
                  currentStep === i + 1 && "bg-[#1E40AF]"
                } ${i + 1 < currentStep ? "bg-[#1E40AF]" : "bg-white"}`}
              >
                {/* {i + 1} */}
              </div>
              <span className="text-[12px] lg:text-base">{step}</span>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white mt-10 lg:mt-20 rounded-xl">
          <div className="border-[#1E40AF] rounded-xl py-5 px-8 lg:px-0 lg:py-10 border-[1px]">
            ...
            <p className="text-center text-[10px] lg:text-[14px]">
              "If you tell us the job title and employer, we'll tailor your
              cover letter accordingly. If you don't specify a job, we'll
              provide a cover letter that you can personalize later."
            </p>
          </div>
          ...
          <div className="mt-5 flex flex-col items-center py-8 px-3 lg:py-10 lg:px-[7%] gap-5 lg:gap-5 justify-center">
            <div className="flex flex-wrap justify-center gap-4 lg:gap-6">
              {skills.map((item, index) => (
                ...
                //     skill?.includes(item) ? "bg-[#1E40AF]" : ""
                //   }`}
                //   onClick={(e) => handleSkil(item)}
                // />
                <input
                  key={index}
                  type="button"
                  value={item + " " + "+"}
                  style={{ transitionProperty: "background-color" }}
                  className={`text-[12px] lg:text-base text-[#000] text-center rounded-md  md:px-3 px-3 lg:py-2 py-2 flex items-center bg-opacity-0 border-[#000] border-[1px] justify-center ${
                    color.includes(item + " " + "+") ? "bg-[#1E40AF]" : ""
                  }`}
                  onClick={(e) => handleSkill(e)}
                />
              ))}
            </div>
          </div>
        ...
        <div className="bg-white mt-10 lg:mt-20 rounded-xl">
          <div className="border-[#1E40AF] rounded-xl py-5 px-8 lg:px-0 lg:py-10 border-[1px]">
            ...
            <p className="text-center text-[10px] lg:text-[14px]">
              Select a recent job that’s relevant to the position you’re
              applying for.
            </p>
          </div>
          ...
          <div className="mt-5 py-8 px-3 lg:py-10 lg:px-[7%] gap-10 lg:gap-5 justify-center">
            <div className=" grid grid-cols-1 lg:grid-cols-3 gap-10">
              ...
              <div className="border-[#1E40AF] rounded-xl p-5 lg:p-8 border-[1px]">
                ...
                <p>
                  You introduce organization and concentration to optimize
                  tasks.
                </p>
              </div>
